...
def ver_alumnos(request):
    ...
    if request.user.is_active:
        avatares = Avatar.objects.filter(user=request.user.id)
        return render(request,"ver_alumnos.html",{"alumnos":alumnos,"url":avatares[0].imagen.url})
...
def ver_profesores(request):
    ...
    if request.user.is_active:
        avatares = Avatar.objects.filter(user=request.user.id)
        return render(request,"ver_profesores.html",{"profesores":profesores,"url":avatares[0].imagen.url})
...
def buscar_resultado_curso(request):
    if request.GET["nombre"]:
        ...
        avatares = Avatar.objects.filter(user=request.user.id)
        return render( request, "resultado_busqueda_curso.html",{"cursos":cursos,"url":avatares[0].imagen.url})
    else:
...
def buscar_resultado_alumno(request):
    if request.GET["nombre"]:
        ...
        avatares = Avatar.objects.filter(user=request.user.id)
        return render( request, "resultado_busqueda_alumno.html",{"alumnos":alumnos,"url":avatares[0].imagen.url})
    else:
...
def buscar_resultado_profesor(request):
    if request.GET["nombre"]:
        ...
        avatares = Avatar.objects.filter(user=request.user.id)
        return render( request, "resultado_busqueda_profesor.html",{"profesores":profesores,"url":avatares[0].imagen.url})
    else:
...
def eliminar_curso(request,id):
    ...
    avatares = Avatar.objects.filter(user=request.user.id)
    return render(request,"ver_cursos.html",{"cursos":curso,"url":avatares[0].imagen.url})
...
def eliminar_alumno(request,id):
    ...
    avatares = Avatar.objects.filter(user=request.user.id)
    return render(request,"ver_alumnos.html",{"alumnos":alumno,"url":avatares[0].imagen.url})

def eliminar_profesor(request,id):
    profesor = Profesor.object.get(id=id)
    profesor.delete()

    profesor = Profesor.objects.all()
    avatares = Avatar.objects.filter(user=request.user.id)
    return render(request,"ver_profesores",{"profesores":profesor,"url":avatares[0].imagen.url})
...
def editar_profesor(request,id):
    ...
    if request.method == "POST":
        ...
        if mi_formulario.is_valid():
            ...
            return render(request,"ver_profesores.html",{"profesores":profesor,"url":avatares[0].imagen.url})
